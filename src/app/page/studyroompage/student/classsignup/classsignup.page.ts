...
import { CourseCreateModalComponent } from '../../admin/course-create-modal/course-create-modal.component';
...
export class ClasssignupPage implements OnInit {
  ...
  constructor(
    ...
    private courseService: VideoService // 서비스 주입
  ) {}
  ...
  async loadCourses() {
    try {
      const response: ApiResponse<CourseResponseData[]> = await firstValueFrom(this.courseService.getAllClasses());
    ...
    modal.onDidDismiss().then(async (result) => {
      if (result.data) {
        try {
          ...
          const response = await firstValueFrom(this.courseService.updateClass(course.id, updatedCourse));
  ...
  async deleteCourse(courseId: number) {
    ...
    try {
      const response: ApiResponse<void> = await firstValueFrom(this.courseService.deleteClass(courseId)); // 숫자를 문자열로 변환하여 삭제 API 호출
  ...
  async joinCourse(courseId: number) {
    ...
    try {
        ...
        const response = await firstValueFrom(
            this.courseService.joinClass(courseId, registrationData)
        );
