...
import { ClassmyResponseData } from 'src/app/models/course/dummy/classmy/classmy-response.interface';
...
export class ClassmyPage implements OnInit {
  ...
  private async checkApprovalStatus(courseId: number): Promise<boolean> {
    ...
    try {
        ...
        console.log('check3:', response.data.course?.id === courseId)
        ...
        const isApproved = Boolean(
          ...
          response.data.course?.id === courseId
        );
